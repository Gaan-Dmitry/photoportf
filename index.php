...
<body>
    ...
    <!-- Секция загрузки (Форма к бэкенду) -->
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Функция проверки кулдауна при загрузке страницы
    function checkCooldown() {
        const lastSubmit = localStorage.getItem('lastRetouchSubmit');
        ...
        if (lastSubmit) {
            ...
            if (timePassed < cooldownTime) {
                ...
                // Запускаем таймер
                updateTimer(lastSubmit);
                timerInterval = setInterval(() => updateTimer(lastSubmit), 1000);

                return true;
            } else {
                // Если время вышло, очищаем хранилище
                localStorage.removeItem('lastRetouchSubmit');
    ...
    function updateTimer(lastSubmitTime) {
    ...
    // Если отправка прошла успешно (перезагрузка страницы с ?success=1)
    ...
    // При обычный отправке (если js срабатывает до того как ушли на сервер)
